...
export async function authenticate(req, res) {
  ...
  try {
    ...
    if (usersSnapshot.empty) {
      logger.warn("User not found in Firestore", { email: decodedToken.email, uid });
    ...
    req.uid = uid;
    logger.info(`${req.method} ${req.url}`, { userId: uid, email: userProfile.email, role: userProfile.role });
    return userProfile;
  } catch (error) {
